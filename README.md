...
**2. 데이터 준비** — clinical CSV는 이미 `dataset/`에 들어있고, 이미지만 받으면 된다

- Google Drive에서 `toy_image_dataset` 다운로드 → 압축 풀어서 `dataset/toy_image_dataset/`에 배치
  (자세한 내용은 [데이터 준비](#데이터-준비) 참고)

**3. DINOv3 에셋 준비** — 로컬 HF 스냅샷을 `assets/dinov3-hf/`에 배치
  (자세한 내용은 [DINOv3 에셋 준비](#dinov3-에셋-준비) 참고)
...
set "COMMON=--resolution 224 --seed 42 --val-ratio 0.2"

%PYTHON% -m train_fusion %COMMON% --fusion-strategy concat --run-name concat_v1
%PYTHON% -m train_fusion %COMMON% --fusion-strategy gated  --run-name gated_v1
%PYTHON% -m train_fusion %COMMON% --fusion-strategy gbdt   --run-name gbdt_v1
```
